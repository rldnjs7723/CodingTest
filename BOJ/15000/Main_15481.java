...
/**
 * 백준 15481번 그래프와 MST
 * 문제 분류: 크루스칼 알고리즘, 최소 공통 조상
 * @author Giwon
 */
public class Main_15481 {
	// 최대 깊이
	...
	public static void main(String[] args) throws IOException {
		...
		for(Edge edge: E) {
			System.out.println(tree.newMST(edge.u, edge.v, edge.w));
		}
	...
	public static class Tree {
		...
		// 특정 조상 노드까지 도달하는 경로 중 가중치가 최대인 값을 탐색 
		public long maxWeight(Node U, Node parent) {
			int max = 0;
			while(U != parent) {
				max = Math.max(max, U.weight);
				U = U.parents[0];
			}
		...
		public long maxWeight(int u, int v) {
			Node parent = this.lca(u, v);
			Node U = nodes[u];
			Node V = nodes[v];
			
			long max = this.maxWeight(U, parent);
			max = Math.max(max, this.maxWeight(V, parent));
		...
		public long newMST(int u, int v, int w) {
			long weight = this.weight - maxWeight(u, v);
			return weight + w;
		}
	}
