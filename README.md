...
3. 용어  
   3-1. 차수: 노드에 연결된 자식 노드의 수 (노드 기준) / 루트 노드의 차수 (트리 기준)  
   3-2. 높이: 루트에서 노드에 이르는 간선의 수 (노드 기준) / 트리에서 노드의 높이 중 가장 큰 값 (트리 기준)
4. 트리 순회  
   4-1. 전위 순회: 부모 노드 -> 왼쪽 자식 -> 오른쪽 자식 순으로 방문  
   4-2. 중위 순회: 왼쪽 자식 -> 부모 노드 -> 오른쪽 자식 순으로 방문  
   4-3. 후위 순회: 왼쪽 자식 -> 오른쪽 자식 -> 부모 노드 순으로 방문
...
1. 이진 탐색 알고리즘의 일종
2. 알고리즘  
   2-1. 각 부분 수열의 길이 값에 해당하는 가장 작은 값들을 기록. 자기 자신이 어떤 위치에 들어가야 하는지 확인할 때 `이분 탐색`으로 Index를 찾음.  
   2-2. 자기 자신의 부분 수열 길이를 저장하는 배열을 따로 설정하고, 이후 마지막에 맨 뒤 (N - 1)부터 탐색하며 길이가 가장 긴 원소부터 차례대로 추출하여 최장 부분 수열을 출력
