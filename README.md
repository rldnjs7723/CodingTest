...
2. Ctrl + Space: 자동완성.
3. Ctrl + H: 전체 검색
4. sysout 입력 후 Ctrl + Space를 누르면 System.out.println()이 입력 [(2558)](https://github.com/rldnjs7723/CodingTest/blob/main/BOJ/2000/Main_2558.java)
5. 디버거
    1. 디버거 설정
...
9. [Graph (그래프 탐색 알고리즘)](#graph-그래프-탐색-알고리즘)
    ...
    6. [Strongly Connected Component (강한 결합 요소)](#strongly-connected-component-강한-결합-요소)
        ...
        1. [Kosaraju's Algorithm (코사라주 알고리즘)](#1-kosarajus-algorithm-코사라주-알고리즘-2150)

        2. [Tarjan's Algorithm (타잔 알고리즘)](#2-tarjans-algorithm-타잔-알고리즘-참고)

        3. [2-SAT](#3-2-sat-11280-11281)

10. [Segment Tree (세그먼트 트리)](#segment-tree-세그먼트-트리)

    1. [Fenwick Tree (Binary Indexed Tree, BIT)](#fenwick-tree-binary-indexed-tree-bit)

    2. [Segment Tree With Lazy Propagation (느리게 갱신되는 세그먼트 트리)](#segment-tree-with-lazy-propagation-느리게-갱신되는-세그먼트-트리)

    3. [Multidimensional Segment Tree (다차원 세그먼트 트리)](#multidimensional-segment-tree-다차원-세그먼트-트리)
...
13. [ETC. (기타 기법)](#etc-기타-기법)
    ...
    9. [Coordinate Compression (좌표 압축)](#coordinate-compression-좌표-압축)
...
2. 공차가 d이고, 첫 항이 a인 등차수열의 n번째 항까지의 합  
   $\frac{n(2a + (n - 1)d)}{2}$
...
1. SCC(Strongly Connected Component): 임의의 한 정점에 대해 다른 모든 정점으로 가는 경로가 존재하는 그래프. (임의의 두 정점 사이의 경로가 존재하는 유향 그래프)

### [1. Kosaraju's Algorithm (코사라주 알고리즘)](#목차) [(2150)](https://github.com/rldnjs7723/CodingTest/blob/main/BOJ/2000/Main_2150.java)

1. 정방향 그래프에서 DFS로 탐색하여 탐색을 종료한 순서대로 Stack에 집어넣기
...
2. 가장 마지막으로 탐색을 종료한 순서대로 DFS를 한 번 더 수행하여 방문한 모든 점을 하나의 SCC로 구성
...
### [2. Tarjan's Algorithm (타잔 알고리즘)](#목차) [(참고)](https://taesung1993.tistory.com/53)

1. DFS를 수행하면서 방문한 순서대로 Stack에 각 정점을 저장하고 방문 순서를 기록
2. 더 이상 방문할 수 있는 노드가 없을 때, 현재 간선 중 자신의 부모 노드로 돌아갈 수 있는 간선이 존재하지 않는 경우 Stack에서 자신보다 위에 위치한 모든 정점과 SCC를 구성 (자신의 모든 자식 노드와 SCC 구성)
...
### [3. 2-SAT](#목차) [(11280)](https://github.com/rldnjs7723/CodingTest/blob/main/BOJ/11000/Main_11280.java) [(11281)](https://github.com/rldnjs7723/CodingTest/blob/main/BOJ/11000/Main_11281.java)

1. 2-SAT (2-Satisfiability Problem)은 N개의 불리언 변수
   $x_1, x_2, ..., x_n$가 있을 때, 2-CNF 식을 true로 만들기위해
   $x_i$를 어떤 값으로 정해야하는지를 구하는 문제이다.
2. 2-CNF (2-Conjunctive Normal Form) 식은 $(x_1 \vee x_2) \wedge (x_3 \vee x_4)$와 같이 괄호로 묶인 하나의 절(Clause)에 2개의 불리언 변수가 논리합으로 연결되어 있고, 각 절은 논리곱으로 연결되어 있는 논리식을 말한다.
3. 2-CNF 식에서 하나의 절 $(x_1 \vee x_2)$이 참이 되는 경우는  
   3-1. $x_1$과 $x_2$이 둘 다 참  
   3-2. $x_1$이 거짓이면 $x_2$가 참  
   3-3. $x_2$이 거짓이면 $x_1$가 참  
   세 가지이다. 이를 코드로 표현하면 다음과 같이 그래프를 구성할 때 사용할 수 있다.
    ```java
    // 절 추가
    ...
    ```
4. 앞서 하나의 절이 참이 되는 경우를 간선으로 표현하여 2-CNF에 대응되는 그래프를 구성하여 SCC를 탐색했을 때, 하나의 SCC 안에 $x_1$이 참이면서 $¬x_1$도 참이 되는 경우가 존재한다면 해당 2-CNF 식은 절대로 참이 될 수 없다.  
   또한, 위와 같은 SCC가 존재하지 않는다면 해당 2-CNF 식은 특정 값에 따라 참으로 만들 수 있음이 증명되어 있다. [(참고)](https://m.blog.naver.com/kks227/220803009418)

5. SCC를 구성한 뒤 해당 2-CNF 식이 참이 되는 불리언 변수 값을 계산하려면 SCC를 위상 정렬한 뒤, 각 변수에서 먼저 나온 불리언 변수가 거짓이 되도록 (not이 참이 되도록) 설정하면 답을 얻을 수 있다.  
   이 때, 타잔을 사용하여 SCC를 탐색했다면 SCC 배열의 역순이 위상 정렬 순서로 나타나는 성질을 이용할 수 있다.
...
## [Segment Tree With Lazy Propagation (느리게 갱신되는 세그먼트 트리)](#목차)

1. 특정 구간에 대해 갱신 작업을 수행해야 할 때 사용하는 기법. 하나의 값을 갱신하는 과정을 N번 반복하면 $O(N log N)$이 되므로, 배열에 대해 선형으로 갱신하는 $O(N)$보다 오래 걸리게 된다.  
   이를 막기 위해 범위가 완전히 포함되는 영역에서는 자기 자신만 값을 갱신하고 자식 노드에 대해서는 lazy 배열에 값을 저장한 뒤 나중에 방문했을 때 값을 갱신해주는 방식으로 $O(log N)$의 시간복잡도를 유지하도록 한다.
2. 트리의 각 노드에 대해서 lazy하게 갱신할 값을 저장하는 것이기 때문에 구현 시 메모리는 기존 세그먼트 트리의 2배를 사용하게 된다.

3. 알고리즘  
   기본적인 구현 방법은 똑같으나, 현재 노드에 도달했다면 lazy하게 갱신해야 하는지 체크하는 과정과 자신의 값을 갱신할 때 자식 노드의 lazy 값을 갱신해줘야 하는 부분이 다르다.
...
## [Multidimensional Segment Tree (다차원 세그먼트 트리)](#목차)

1. 세그먼트 트리를 이차원 배열로 구성하여 세그먼트 트리의 노드가 다른 세그먼트 트리를 품고 있는 형태로 구현하는 문제. [(참고)](https://m.blog.naver.com/PostView.naver?isHttpsRedirect=true&blogId=jh20s&logNo=221351546136)
2. 시간 복잡도: $O(log N * log N)$
3. 알고리즘

    1. 전체 구간 합 구하기  
       2차원 배열을 입력 받아 각 행별로 세그먼트 트리를 구성한 뒤, 부모 노드에서 자식 트리의 값을 참조하여 값을 구성하도록 구현
    ...
    2. 구간 합 계산하기  
       구간 합 계산은 일반적인 세그먼트 트리에서 구간 합 계산하는 과정을 두 번 재귀적으로 반복하여 수행하면 된다.
    ...
    3. 특정 원소 값 수정하기  
       init과 다르게 트리의 모든 값을 수정하지 않기 위해서 자식 노드를 갱신했을 때 최하단의 Index 값을 리턴받도록 구현하여, 해당 노드의 부모 노드만 갱신하도록 구현해야 한다.
...
4. 모든 경우에서 트라이가 더 빠르게 작동하는 것은 아님. 특정 단어가 집합에 존재하는지만을 확인해야 한다면 HashSet을 사용하는 것이 더 빠를 수 있음. [(19585)](https://github.com/rldnjs7723/CodingTest/blob/main/BOJ/19000/Main_19585.java)
...
3. 비트 연산자
    ...
    3. 비트 제거할 때: & (~bit) 연산자
    4. 비트 반전시킬 때: ~ 연산자
    5. 특정 위치 비트만 반전시킬 때: ^ 연산자
    6. 비트 가득 찬 상태와 비교할 때 XOR 연산자 ^도 가끔 사용
...
## [Coordinate Compression](#목차) [(좌표 압축)](https://github.com/rldnjs7723/CodingTest/blob/main/Ideas/CoordinateCompression.md)

## [Meet In The Middle (중간에서 만나기)](#목차)

## [Bipartite Matching (이분 매칭)](#목차)

## [Convex Hull (볼록 껍질)](#목차)
